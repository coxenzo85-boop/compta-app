...
# ==============================================================================
# 1. CONFIGURATION & ÉTAT (DOIT TOUJOURS ÊTRE EN PREMIER)
# ==============================================================================
...
# ==============================================================================
# 2. FONCTIONS UTILITAIRES (DÉFINITION)
# ==============================================================================
    ...
    if sh:
        try:
            ...
            if not df_sim.empty:
                ...
                safe_total = total_coef.replace(0, 1)
                s1['Moy'] = ((s1['Note_CC']*s1['Coef_CC']) + (s1['Note_Partiel']*s1['Coef_Partiel'])) / safe_total
                valid = total_coef > 0
    ...
    with cr:
        ...
        if sh:
            ...
            if not edited_exams.empty:
                ...
                except: st.caption("Format date invalide")
...
def focus_page(sh):
    ...
    with st.expander("🎵 Ambiance Lofi (Apple Music)", expanded=True):
        ...
        """
        st.components.v1.html(embed_code, height=180)
